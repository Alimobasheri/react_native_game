...
export function useEntityMemoizedValue<E extends Record<string, any>, T>(
  ...
  if (!rnsgeContext) {
    throw new Error('useEntity must be used within a RNSGEContext');
  }
...
export const useFrameMemo = <E extends Record<string, any>, T>(
  ...
  }, [equalityCheck, factory, framesRef]);
...
describe('Entity class', () => {
  ...
  it('should emit an update event when the data is reassigned but not changed', (done) => {
    const entity = new Entity({ foo: 'bar' });
    entity.addListener(EntityUpdateEvent, (event) => {
      expect(event.prev).toStrictEqual({ foo: 'bar' });
      expect(event.next).toStrictEqual({ foo: 'bar' });
      done();
    });
    entity.data = { foo: 'bar' };
  });

  it('should not emit an update event when the data is not changed, because of strictEqual comparison', (done) => {
    const entity = new Entity(
      { foo: 'bar' },
      EntityChangeComparison.StrictEqual
    );
    entity.addListener(EntityUpdateEvent, (event) => {
      done('should not emit an update event when the data is not changed');
    });
    entity.data = { foo: 'bar' };
    setTimeout(() => done(), 0);
  });
...
/**
 * Represents the event arguments for an entity update event.
 *
 * @template T The type of data associated with the entity.
 */
export class EntityUpdateEventArgs<T extends Record<string, any>> {
  /**
   * The previous data associated with the entity.
   *
   * @type {T}
   */
  public prev: T;
  ...
  /**
   * The new data associated with the entity.
   *
   * @type {T}
   */
  public next: T;
  ...
  /**
   * Creates a new entity update event arguments instance.
   *
   * @param {T} prev The previous data associated with the entity.
   * @param {T} next The new data associated with the entity.
   */
...
export class Entity<T extends Record<string, any>> extends EventEmitter {
   ...
   */
  constructor(data: T, comparison?: EntityChangeComparison) {
    super();
    this._id = uid();
    this._data = data;
  ...
  public set data(data: any) {
    ...
    const args: EntityUpdateEventArgs<T> = new EntityUpdateEventArgs<T>(
      this._data,
      data
    );
