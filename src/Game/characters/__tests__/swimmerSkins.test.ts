...
describe('swimmerSkins', () => {
  ...
  it('builds body, eyes, and hair crest layers from aqua-sprout skin art keys', () => {
    ...
    expect(layers[1].image).toBe(SWIMMER_CHARACTER_IMAGE.aquaSproutEyes);
    expect(layers[2].image).toBe(SWIMMER_CHARACTER_IMAGE.aquaSproutHair);
    expect(layers[0].shape).toMatchObject({ width: 48, height: 120 });

    const featureSize = getFeatureMeshSize(AQUA_SPROUT_SKIN.feature!, 48);
    expect(layers[1].shape).toMatchObject(featureSize);
    expect(layers[1].position?.y).toBe(
      getFeatureRestOffsetY(AQUA_SPROUT_SKIN.feature!, 120)
    );
    expect(layers[1].position?.y).toBeLessThan(0);

    const accessorySize = getAccessoryMeshSize(AQUA_SPROUT_SKIN, 48, 120);
    expect(layers[2].shape).toMatchObject(accessorySize);
    expect(layers[2].position?.y).toBeLessThan(layers[1].position!.y!);
    expect(getSwimmerAccessoryLayerIndex(AQUA_SPROUT_SKIN)).toBe(2);
  });
